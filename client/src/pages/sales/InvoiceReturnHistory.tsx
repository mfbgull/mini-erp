...
export default function InvoiceReturnHistory() {
  ...
  const { data: returns = [], isLoading } = useQuery({
    queryKey: ['invoice-returns', filters],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (filters.start_date) params.append('start_date', filters.start_date);
      if (filters.end_date) params.append('end_date', filters.end_date);
      if (filters.item_id) params.append('item_id', filters.item_id);
      const response = await api.get(`/invoices/returns?${params.toString()}`);
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      filter: 'agNumberColumnFilter',
      flex: 1,
      valueFormatter: (params: any) => fmtCurrency(Number(params.value) || 0),
    },
    {
    ...
    },
    {
      ...
      valueFormatter: (params: any) => {
        ...
        // Strip the "Stock reversed - Invoice ... returned" prefix if present
        const cleaned = params.value.replace(/^Stock reversed - [^:]* returned\s*/i, '');
        return cleaned || '—';
      },
    },
    {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            View history of all invoice return transactions
          </p>
        </div>
        ...
        <MiniERPGrid
          ...
          noRowsOverlayComponent={() => (
            <div className="no-rows" style={{ textAlign: 'center', padding: '40px', color: '#6c757d' }}>
              ...
              <p>No invoice returns match the current filters.</p>
            </div>
          )}
        />
      )}
