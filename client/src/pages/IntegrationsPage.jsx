        ...
        <div className="settings-section">
          <div className="section-header">
            <div className="flex items-center justify-between">
              <div>
                <h2>
                  <Mail size={24} className="icon-valign-middle icon-mr-sm" />
                  Email Service (SendGrid)
                </h2>
                <p className="section-description">
                  Send invoices, POs, and notifications via email
                </p>
              </div>
              {settings?.email && (
                <span
                  className={`badge ${getBadgeClass(settings.email.configured)}`}
                >
                  {getStatusBadge(settings.email.configured)}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="form-input-group">
              <label className="form-label">
                <input
                  ...
                  onChange={(e) =>
                    handleEnableChange("email", e.target.checked)
                  }
                />
                Enable Email Service
              </label>
              <small className="form-help-text">
                Send emails automatically on invoice/PO creation
              </small>
            </div>
            {emailExpanded && (
              <>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2>
                  <MessageSquare
                    size={24}
                    className="icon-valign-middle icon-mr-sm"
                  />
                  SMS Notifications (Twilio)
                </h2>
                <p className="section-description">
                  Send SMS alerts for low stock, payments, and orders
                </p>
              </div>
              {settings?.notifications && (
                <span
                  className={`badge ${getBadgeClass(settings.notifications.configured)}`}
                >
                  {getStatusBadge(settings.notifications.configured)}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="form-input-group">
              <label className="form-label">
                <input
                  ...
                  onChange={(e) =>
                    handleEnableChange("notification", e.target.checked)
                  }
                />
        ...
        <div className="settings-section">
          <div className="section-header">
            <div className="flex items-center justify-between">
              <div>
                <h2>
                  <Cloud size={24} className="icon-valign-middle icon-mr-sm" />
                  Weather (Weatherstack)
                </h2>
                <p className="section-description">
                  Get weather data for delivery planning
                </p>
              </div>
              {settings?.weather && (
                <span
                  className={`badge ${getBadgeClass(settings.weather.configured)}`}
                >
                  {getStatusBadge(settings.weather.configured)}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="form-input-group">
              <label className="form-label">
                <input
                  ...
                  onChange={(e) =>
                    handleEnableChange("weather", e.target.checked)
                  }
                />
                Enable Weather Service
              </label>
              <small className="form-help-text">
                Enable weather data for deliveries
              </small>
            </div>
            {weatherExpanded && (
              <>
              ...
              <div>
                <h2>
                  <CheckCircle
                    ...
                    className="icon-valign-middle icon-mr-sm"
                  />
                  Data Validation (Numverify)
                </h2>
                <p className="section-description">
                  Validate phone numbers for customers and suppliers
                </p>
              </div>
              {settings?.validation && (
                <span
                  className={`badge ${getBadgeClass(settings.validation.configured)}`}
                >
                  {getStatusBadge(settings.validation.configured)}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="form-input-group">
              <label className="form-label">
                <input
                  ...
                  onChange={(e) =>
                    handleEnableChange("validation", e.target.checked)
                  }
                />
        ...
        <div className="settings-section">
          <div className="section-header">
            <div className="flex items-center justify-between">
              ...
              {settings?.currency && (
                <span
                  className={`badge ${getBadgeClass(settings.currency.configured)}`}
                >
                  {getStatusBadge(settings.currency.configured)}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="form-input-group">
              <label className="form-label">
                <input
                  ...
                  onChange={(e) =>
                    handleEnableChange("currency", e.target.checked)
                  }
                />
        ...
        <div className="settings-section">
          <div className="section-header">
            <div className="flex items-center justify-between">
              ...
              {settings?.tax && (
                <span
                  className={`badge ${getBadgeClass(settings.tax.configured)}`}
                >
                  {getStatusBadge(settings.tax.configured)}
                </span>
              )}
            </div>
          </div>
          <div className="settings-grid">
            <div className="form-input-group">
              ...
              <small className="form-help-text">
                Calculate tax automatically on invoices
              </small>
            </div>
            {taxExpanded && (
              <>
                <FormInput
                  ...
                  onChange={(e) =>
                    setTaxConfig({ ...taxConfig, apiKey: e.target.value })
                  }
                ...
                <FormInput
                  ...
                  onChange={(e) =>
                    setTaxConfig({ ...taxConfig, zipCode: e.target.value })
                  }
