...
import type { InventoryItemOption } from '../../utils/purchaseOrderTypes';
...
export function POSearchableCellEditing({
  ...
  const getPurchasableItems = useCallback(() => {
    return inventoryItems.filter(item =>
      item.is_purchased === true || (!item.is_raw_material && !item.is_manufactured)
    ).slice(0, 10);
  }, [inventoryItems]);
  ...
  const focusTargetCell = useCallback((targetItemId: number, targetField: string) => {
    setTimeout(() => {
      if (targetField === 'name') {
        onEditingCell(`${targetItemId}-name`);
        setTimeout(() => {
  ...
  const openDropdown = useCallback(() => {
    ...
    setLocalFilteredItems(purchasable);
    setShowDropdown(purchasable.length > 0);
    setSelectedIndex(purchasable.length > 0 ? 0 : -1);
  }, [inventoryItems, getPurchasableItems]);

  const handleSave = useCallback(() => {
    if (tempValue !== value) {
      onUpdateItem(itemId, 'name', tempValue);
    }
    setShowDropdown(false);
    setLocalFilteredItems([]);
    onEditingCell(null);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (searchValue.trim()) {
      const matches = purchasable.filter(item =>
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.ctrlKey && e.key === 'ArrowDown') {
      ...
      onEditingCell(`${newId}-name`);
      focusTargetCell(newId, 'name');
  ...
  const inputRect = document.querySelector(`[data-cell-id="${itemId}-name"] input`)?.getBoundingClientRect();
