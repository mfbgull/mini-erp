...
async function getCsrfToken(authCookie: string): Promise<{ cookie: string; token: string }> {
  const res = await request(app)
    .get('/api/customers')
    .set('Cookie', authCookie);
  const cookies = res.headers['set-cookie'];
  if (!cookies) return { cookie: '', token: '' };
  const csrfCookie = (Array.isArray(cookies) ? cookies : [cookies])
    .find((c: string) => c.startsWith('csrf-token='));
  if (!csrfCookie) return { cookie: '', token: '' };
...
describe('JWT Security', () => {
  it('rejects tampered token (modified payload)', async () => {
    const fakeToken = jwt.sign(
      { id: 1, username: 'hacker', email: 'x@y.z', role: 'admin' },
      'wrong-secret',
      { algorithm: 'HS256' }
    );
    const res = await request(app)
  ...
  it('rejects token with wrong issuer', async () => {
    const fakeToken = jwt.sign(
      ...
      { algorithm: 'HS256', issuer: 'evil-app' }
    );
    const res = await request(app)
      .get('/api/customers')
      .set('Cookie', `token=${fakeToken}`);
  ...
  it('rejects token with wrong audience', async () => {
    const fakeToken = jwt.sign(
      { id: 1, username: 'admin', email: 'a@b.c', role: 'admin' },
      process.env.JWT_SECRET!,
      { algorithm: 'HS256', audience: 'evil-client' }
    );
    const res = await request(app)
      .get('/api/customers')
      .set('Cookie', `token=${fakeToken}`);
  ...
  it('rejects expired token', async () => {
    const expiredToken = jwt.sign(
      { id: 1, username: 'admin', email: 'a@b.c', role: 'admin' },
      process.env.JWT_SECRET!,
      { algorithm: 'HS256', expiresIn: '-1h', issuer: 'mini-erp', audience: 'mini-erp-client' }
    );
    const res = await request(app)
      .get('/api/customers')
...
describe('Security Headers (Helmet)', () => {
  ...
  it('includes Strict-Transport-Security or Referrer-Policy header', async () => {
    const res = await request(app).get('/health');
    expect(
      res.headers['strict-transport-security'] || res.headers['referrer-policy']
    ).toBeDefined();
  });
});
