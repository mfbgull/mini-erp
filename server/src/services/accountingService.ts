...
export interface PostedEntry {
  journal_entry_id: number;       // logical grouping id for the lines
  line_count: number;
  total_debit: number;
  total_credit: number;
}
...
export interface AccountBalance {
  ...
  total_debit: number;
  total_credit: number;
  balance: number;                // signed: positive = debit-normal, negative = credit-normal
  text_code: string | null;
}
...
export class AccountingService {
  ...
  static listAccounts(db: Database.Database, includeInactive = false): Account[] {
  ...
  static getAccountByCode(db: Database.Database, code: string): Account | undefined {
    return db.prepare(`SELECT * FROM chart_of_accounts WHERE code = ?`).get(code) as Account | undefined;
  }

  static getAccountByTextCode(db: Database.Database, textCode: string): Account | undefined {
    return db.prepare(`SELECT * FROM chart_of_accounts WHERE text_code = ?`).get(textCode) as Account | undefined;
  }
  ...
  static getAccountBalance(
    ...
    const acct = db.prepare(`SELECT * FROM chart_of_accounts WHERE id = ?`).get(accountId) as Account | undefined;
    if (!acct) {
   ...
   */
  static getAllAccountBalances(db: Database.Database, asOfDate: string): AccountBalance[] {
   ...
   */
  static postEntry(db: Database.Database, input: PostEntryInput): PostedEntry {
    ...
    `).get(input.entry_date, input.entry_date) as { id: number; period_name: string } | undefined;

    if (!openPeriod) {
      throw new Error(
        `No open accounting period covers ${input.entry_date}. ` +
  ...
  static openPeriod(db: Database.Database, periodName: string, startDate: string, endDate: string): number {
